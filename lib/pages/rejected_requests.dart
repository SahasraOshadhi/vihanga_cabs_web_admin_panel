...
class _RejectedRequestPageState extends State<RejectedRequestPage> {
  @override
...
class _RideRequestCardState extends State<RideRequestCard> {
                    ...
                    ElevatedButton(
                      onPressed: () {
                        if (selectedDriver != null) {
                          _reassignDriver(widget.rideRequest.id, selectedDriver!);
                        }
                      },
  ...
  void _reassignDriver(String rideRequestId, String driverId) {
    FirebaseFirestore.instance.collection('ride_requests').doc(rideRequestId).update({
      'assignedDriver': driverId,
      'assigned': 'yes',
      'acceptedByDriver': FieldValue.delete(), // Remove the rejection flag
    }).then((_) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Driver reassigned successfully')));
    }).catchError((error) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to reassign driver: $error')));
    });
  }
}
